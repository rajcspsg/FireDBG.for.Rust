...
use anyhow::Result;
use firedbg_rust_debugger::{Bytes, Debugger, Event, EventStream, typename::wildcard_match};
    ...
    for i in 0..102 {
        ...
        match event {
            Event::FunctionReturn {
                ...
                // LLDB often reads wrong 128-bit scalar payloads; still require correct Option variant.
                match i {
                    52 | 56 => {
                        if return_value != expected {
                            let (pre, suf) = match i {
                                52 => (
                                    "core::option::Option::<i128>::Some(",
                                    "i128)",
                                ),
                                56 => (
                                    "core::option::Option::<u128>::Some(",
                                    "u128)",
                                ),
                                _ => unreachable!(),
                            };
                            assert!(
                                return_value.starts_with(pre) && return_value.ends_with(suf),
                                "assertion failed:\n  left: `{return_value}`\n right: `{expected}`"
                            );
                        }
                    }
                    // `pistons` may print as `(?)` when the vec cannot be expanded.
                    84 => {
                        let tmpl = r#"core::option::Option::<return_option::Car>::Some(return_option::Car { brand: "Ford", engine: return_option::Engine { config: return_option::EngineConfig::Inline { i: 4i32 }, pistons:.. }, gearbox: return_option::Gearbox::Manual })"#;
                        if !wildcard_match(tmpl, &return_value) {
                            assert_eq!(return_value, expected);
                        }
                    }
                    _ => assert_eq!(return_value, expected),
                }
